...
class CanvasAPI:
    ...
    def get_assignment_submissions(self, course_id: int, assignment_id: int, 
        ...
        """
        print(f"🔍 get_assignment_submissions called with course_id={course_id}, assignment_id={assignment_id}")
        ...
        while True:
            ...
            try:
                ...
                print(f"🔍 Page {page} returned {len(submissions)} submissions")
                
                if not submissions:
                    print(f"🔍 No more submissions on page {page}, stopping")
                    break
                    
                all_submissions.extend(submissions)
                
                # Debug: Show first submission structure
                if page == 1 and submissions:
                    first_sub = submissions[0]
                    print(f"🔍 First submission structure:")
                    print(f"  - ID: {first_sub.get('id', 'no-id')}")
                    print(f"  - User ID: {first_sub.get('user_id', 'no-user-id')}")
                    print(f"  - Workflow state: {first_sub.get('workflow_state', 'no-state')}")
                    print(f"  - Has attachments: {bool(first_sub.get('attachments'))}")
                    print(f"  - Has body: {bool(first_sub.get('body'))}")
                    print(f"  - User info: {first_sub.get('user', {})}")
                
                # Check if there are more pages
                if len(submissions) < params['per_page']:
                    print(f"🔍 Page {page} has fewer than {params['per_page']} items, this is the last page")
                    break
                    
                page += 1
                
            except Exception as e:
                print(f"❌ Error retrieving submissions page {page}: {e}")
                break
    ...
    def download_submissions_bulk(self, course_id: int, assignment_id: int, 
                                download_dir: str, anonymizer: StudentAnonymizer, 
        ...
        """
        print(f"🔍 download_submissions_bulk called with course_id={course_id}, assignment_id={assignment_id}")
        print(f"🔍 download_dir={download_dir}")
        
        os.makedirs(download_dir, exist_ok=True)
        
        print(f"📊 Getting submissions from Canvas...")
        submissions = self.get_assignment_submissions(course_id, assignment_id)
        print(f"📊 Retrieved {len(submissions)} submissions from Canvas")
        ...
        for i, submission in enumerate(submissions):
            ...
            if user_files:
                ...
                print(f"  - ✅ Total files for {anon_name}: {len(user_files)}")
            else:
                print(f"  - ⚠️ No files saved for {anon_name}")
        
        print(f"📊 Final result: Downloaded files for {len(downloaded_files)} students")
...
class TwoStepCanvasGrading:
    ...
    def step1_download_and_grade(self, course_id: int, assignment_id: int, 
        ...
        try:
            ...
            if progress_callback:
                progress_callback(45, "Loading grading rubric...")
            ...
            if progress_callback:
                progress_callback(50, "Grading submissions with AI...")
            
            print("🤖 Grading submissions with ChatGPT (anonymized)...")
            
            grading_results = {}
            total_students = len(downloaded_files)
            processed_students = 0
            
            print(f"🔍 Starting grading loop for {total_students} students with downloaded files")
            if log_callback:
                log_callback(f"🔍 Starting grading loop for {total_students} students")
            
            for user_id, file_paths in downloaded_files.items():
                print(f"🔍 Processing user {user_id} with {len(file_paths)} files")
                if log_callback:
                    log_callback(f"🔍 Processing user {user_id} with {len(file_paths)} files")
                try:
                    ...
                    for i, file_path in enumerate(file_paths):
                        ...
                        if not os.path.exists(file_path_str):
                            ...
                            if log_callback:
                                log_callback(f"    ⚠️ File does not exist: {os.path.basename(file_path_str)}")
                            continue
                        ...
                        if file_path_str.lower().endswith(('.txt', '.docx', '.pdf', '.html', '.odt', '.rtf')):
                            try:
                                ...
                                if file_ext.endswith('.txt'):
                                    ...
                                    try:
                                        ...
                                        if file_ext.endswith('.docx'):
                                            ...
                                            # For ODT files, try basic text extraction
                                            try:
                                                # ODT files are actually zip files containing XML
                                                # For now, we'll use a simple approach
                                                import zipfile
                                                import xml.etree.ElementTree as ET
                                                
                                                print(f"    📄 Attempting ODT text extraction: {file_path_str}")
                                                
                                                try:
                                                    # Try to extract text from ODT using zipfile
                                                    with zipfile.ZipFile(file_path_str, 'r') as odt_zip:
                                                        ...
                                                        if 'content.xml' in odt_zip.namelist():
                                                            ...
                                                            print(f"    ✅ Extracted {len(content)} characters from ODT")
                                                        else:
                                                            raise ValueError("No content.xml found in ODT file")
                                                except Exception as odt_error:
                                                    print(f"    ⚠️ ODT extraction failed, trying text fallback: {odt_error}")
                                                    # Fallback: try to read as text (might get some content)
                                                    try:
                                                        with open(file_path_str, 'r', encoding='utf-8', errors='ignore') as f:
                                                            content = f.read()
                                                        print(f"    ⚠️ Used text fallback for ODT, extracted {len(content)} characters")
                                                    except Exception:
                                                        print(f"    ❌ Could not process ODT file: {file_path_str}")
                                                        continue
                                                        
                                            except Exception as odt_outer_error:
                                                print(f"    ❌ ODT processing failed completely: {odt_outer_error}")
                                                continue
                                ...
                                if log_callback:
                                    log_callback(f"    ✅ Graded {anon_name}: {grade_result.get('overall_score', 0):.1f}/{grade_result.get('max_possible_score', 100)}")
                                
                            except Exception as e:
                                print(f"    ❌ Error grading file {file_path_str}: {e}")
                                if log_callback:
                                    log_callback(f"    ❌ Error grading file: {e}")
                                import traceback
                            ...
                            if log_callback:
                                log_callback(f"    ⚠️ Skipping non-document file: {os.path.basename(file_path_str)}")
                    
                    print(f"🔍 Finished processing files for {anon_name}, got {len(student_grades)} grades")
                    if log_callback:
                        log_callback(f"🔍 Finished processing {anon_name}, got {len(student_grades)} grades")
                    ...
                    if student_grades:
                        ...
                        print(f"🔍 Normalized grade for {anon_name}: {normalized_grade}")
                        ...
                        print(f"✅ Successfully stored grade for {anon_name}: {normalized_grade['total_score']:.1f}/{normalized_grade['max_score']}")
                        
                        # Update progress during grading
                        processed_students += 1
                        if progress_callback and total_students > 0:
                            # Progress from 50% to 85% during grading
                            grading_progress = 50 + (35 * processed_students / total_students)
                            progress_callback(int(grading_progress), f"Graded {processed_students}/{total_students} submissions...")
                    else:
                        print(f"❌ No grades generated for {anon_name} - student_grades list is empty")
                    ...
                    if progress_callback and total_students > 0:
                        grading_progress = 50 + (35 * processed_students / total_students)
                        progress_callback(int(grading_progress), f"Processed {processed_students}/{total_students} submissions...")
            
            # Step 1c: Create review spreadsheet with real names
            if progress_callback:
                progress_callback(90, "Creating review spreadsheet...")
            ...
            if progress_callback:
                progress_callback(95, "Finalizing setup...")
            ...
            if progress_callback:
                progress_callback(100, "Step 1 complete!")
